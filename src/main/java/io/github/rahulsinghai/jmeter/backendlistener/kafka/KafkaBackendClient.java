...
public class KafkaBackendClient extends AbstractBackendListenerClient {
  ...
  static {
    ...
    // No guarantee can be made that the server has received the record in this case, and the <code>retries</code> configuration will not take effect (as the client won't generally know of any failures).
    ...
    // SSL, SSLv2 and SSLv3 may be supported in older JVMs, but their usage is discouraged due to known security vulnerabilities.
    ...
    // and the time allowed for retriable send failures. The producer may report failure to send a record earlier than this config if either an unrecoverable error is encountered,
    ...
    // Setting <code>linger.ms=5</code>, for example, would have the effect of reducing the number of requests sent but would add up to 5ms of latency to records sent in the absence of load.
  ...
  public void setupTest(BackendListenerContext context) throws Exception {
    ...
    this.buildNumber = (JMeterUtils.getProperty(KafkaBackendClient.BUILD_NUMBER) != null
        && !JMeterUtils.getProperty(KafkaBackendClient.BUILD_NUMBER).trim().equals(""))
        ? Integer.parseInt(JMeterUtils.getProperty(KafkaBackendClient.BUILD_NUMBER)) : 0;

    Properties props = new Properties();
    props.put(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG,
        context.getParameter(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG));
    props.put(ProducerConfig.CLIENT_ID_CONFIG,
        context.getParameter(ProducerConfig.CLIENT_ID_CONFIG));
    ...
    props.put(ProducerConfig.CLIENT_DNS_LOOKUP_CONFIG,
    ...
    props.put(ProducerConfig.LINGER_MS_CONFIG,
        context.getParameter(ProducerConfig.LINGER_MS_CONFIG));
   ...
   */
  private void convertParameterToSet(BackendListenerContext context, String parameter,
      Set<String> set) {
    String[] array =
        (context.getParameter(parameter).contains(";")) ? context.getParameter(parameter).split(";")
  ...
  public void handleSampleResults(List<SampleResult> results, BackendListenerContext context) {
    for (SampleResult sr : results) {
      JSONMetric metric = new JSONMetric(sr, context.getParameter(KAFKA_TEST_MODE),
          context.getParameter(KAFKA_TIMESTAMP), this.buildNumber,
          context.getBooleanParameter(KAFKA_PARSE_REQ_HEADERS, false),
          context.getBooleanParameter(KAFKA_PARSE_RES_HEADERS, false), fields);
  ...
  private boolean validateSample(BackendListenerContext context, SampleResult sr) {
    ...
    if (sr.isSuccessful() && context.getParameter(KAFKA_TEST_MODE).trim().equalsIgnoreCase("error")
        && valid) {
      valid = false;
    }
